...
import { Loader2, Mail, Lock, User, ArrowLeft, Zap } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";

export const Route = createFileRoute("/auth")({
  head: () => ({ meta: [{ title: "Connexion — ẞoost-by Ecr_aaM" }] }),
  component: AuthPage,
});
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="px-4 pt-4">
        <Button asChild variant="ghost" size="sm" className="gap-1.5"><Link to="/"><ArrowLeft className="h-4 w-4" />Accueil</Link></Button>
      </div>
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md fade-in">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-muted-foreground">Connectez-vous pour commander</p>
          </div>

          <div className="glass-strong rounded-2xl p-5">
            <Button onClick={googleSignIn} disabled={busy} variant="outline" className="w-full border-border bg-secondary/40 mb-3 h-11">
            ...
            <div className="relative my-3 text-center">
              ...
              <span className="relative bg-card px-2 text-[10px] uppercase tracking-wider text-muted-foreground">ou</span>
            </div>
